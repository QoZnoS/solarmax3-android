...
public static class NGUITools
{
	...
	public static void NormalizeWidgetDepths(UIWidget[] list)
	{
		...
		if (num > 0)
		{
			if (NGUITools.cache0 == null)
			{
				NGUITools.cache0 = new Comparison<UIWidget>(UIWidget.FullCompareFunc);
			}
			Array.Sort<UIWidget>(list, NGUITools.cache0);
			int num2 = 0;
	...
	public static void NormalizePanelDepths()
	{
		...
		if (num > 0)
		{
			...
			if (NGUITools.cache1 == null)
			{
				NGUITools.cache1 = new Comparison<UIPanel>(UIPanel.CompareFunc);
			}
			Array.Sort<UIPanel>(array2, NGUITools.cache1);
			int num2 = 0;
	...
	public static UIPanel CreateUI(Transform trans, bool advanced3D, int layer)
	{
		...
		if (uipanel2 == null)
		{
			...
			uiroot.gameObject.AddComponent<UICamera>();
	...
	{
		return cam.GetSides(Mathf.Lerp(cam.nearClipPlane, cam.farClipPlane, 0.5f), null, false);
	}

	public static Vector3[] GetSides(this Camera cam, float depth)
	{
		return cam.GetSides(depth, null, false);
	}

	public static Vector3[] GetSides(this Camera cam, Transform relativeTo, bool showInSafeArea = false)
	{
		return cam.GetSides(Mathf.Lerp(cam.nearClipPlane, cam.farClipPlane, 0.5f), relativeTo, showInSafeArea);
	}

	public static Vector3[] GetSides(this Camera cam, float depth, Transform relativeTo, bool showInSafeArea = false)
	{
		if (cam.orthographic)
		{
			float num = 1f;
			...
			float num3 = orthographicSize * num;
			float y = -orthographicSize;
			float y2 = orthographicSize;
			Rect rect = cam.rect;
			Vector2 screenSize = NGUITools.screenSize;
			float num4 = screenSize.x / screenSize.y;
			num4 *= rect.width / rect.height;
			num2 *= num4;
			num3 *= num4;
			Transform transform = cam.transform;
			Quaternion rotation = transform.rotation;
			Vector3 position = transform.position;
			int num5 = Mathf.RoundToInt(screenSize.x);
			int num6 = Mathf.RoundToInt(screenSize.y);
			if ((num5 & 1) == 1)
			{
			...
			NGUITools.mSides[0] = rotation * new Vector3(num2, 0f, depth) + position;
			NGUITools.mSides[1] = rotation * new Vector3(0f, y2, depth) + position;
			NGUITools.mSides[2] = rotation * new Vector3(num3, 0f, depth) + position;
	...
	public static float Simulate_iPhoneXScale
	{
		get
		{
			if (!NGUITools.Simulate_X)
			{
				return 1f;
			}
			float num = 812f;
			float num2 = 132f;
			return (num - num2 * 2f) / num;
		}
	}
	...
	public static T Draw<T>(string id, NGUITools.OnInitFunc<T> onInit = null) where T : UIWidget
	{
		...
		// 修正
		uiwidget = NGUITools.AddWidget<UIWidget>(mGo, int.MaxValue);
		uiwidget.name = id;
	...
	[CompilerGenerated]
	private static Comparison<UIWidget> cache0;

	[CompilerGenerated]
	private static Comparison<UIPanel> cache1;
