...
public class UIEventListener : MonoBehaviour
{
	...
	protected void PressScale(bool scale)
	{
		...
		int px = (!scale) ? 0 : 20;
		if (this.cacheAutoZoomWidget == null)
		{
			UIWidget x = base.GetComponent<UITexture>();
			if (x == null)
			{
				x = base.GetComponent<UISprite>();
			}
			if (x == null)
			{
				x = base.GetComponentInChildren<UITexture>();
			}
			if (x == null)
			{
				x = base.GetComponentInChildren<UISprite>();
			}
			if (x == null)
			{
				x = base.GetComponent<UIWidget>();
			}
			this.cacheAutoZoomWidget = x;
		}
		if (this.cacheAutoZoomWidget != null)
		{
			this.cacheAutoZoomWidget.autoResizeBoxCollider = !scale;
			this.cacheAutoZoomWidget.ManualScale(px);
		}
	}
	...
	private UIWidget cacheAutoZoomWidget;
