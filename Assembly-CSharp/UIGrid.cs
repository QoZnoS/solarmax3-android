	...
	public List<Transform> GetChildList()
	{
		...
		if (this.sorting != UIGrid.Sorting.None && this.arrangement != UIGrid.Arrangement.CellSnap)
		{
			if (this.sorting == UIGrid.Sorting.Alphabetic)
			{
				...
				if (UIGrid.cache0 == null)
				{
					UIGrid.cache0 = new Comparison<Transform>(UIGrid.SortByName);
				}
				list2.Sort(UIGrid.cache0);
			}
			else if (this.sorting == UIGrid.Sorting.Horizontal)
			{
				List<Transform> list3 = list;
				if (UIGrid.cache1 == null)
				{
					UIGrid.cache1 = new Comparison<Transform>(UIGrid.SortHorizontal);
				}
				list3.Sort(UIGrid.cache1);
			}
			else if (this.sorting == UIGrid.Sorting.Vertical)
			{
				List<Transform> list4 = list;
				if (UIGrid.cache2 == null)
				{
					UIGrid.cache2 = new Comparison<Transform>(UIGrid.SortVertical);
				}
				list4.Sort(UIGrid.cache2);
			}
