...
public class CustomSelectWindow : BaseWindow
{
	private void Awake()
	{
	...
	private void Update()
	{
		...
		if (!this.bIsShowMap)
		{
			float num4 = this.changePageLength * (float)(-(float)CustomSelectWindow.selectMapIndex);
		...
		if (num5 != CustomSelectWindow.selectMapIndex && num5 >= 0 && num5 <= this.MapIndexMax)
		{
	...
	public override void OnShow()
	{
		...
		text = Solarmax.Singleton<GameVariableConfigProvider>.Instance.GetData(1);
		...
		if (CustomSelectWindow.selectMapIndex > this.MapIndexMax)
		{
			CustomSelectWindow.selectMapIndex = -1;
		}
		if (CustomSelectWindow.selectMapIndex == -1)
		{
			if (this.MapIndexMax == this.mapList.Count - 1)
			{
				CustomSelectWindow.selectMapIndex = this.MapIndexMax;
				this.PlayEnter(false);
			}
			else
			{
				CustomSelectWindow.selectMapIndex = 0;
				this.PlayEnter(false);
			}
		}
		else
		{
			this.PlayEnter(false);
			if (CustomSelectWindow.selectMapIndex == this.MapIndexMax - 1 && CustomSelectWindow.selectMapIndex < this.mapList.Count - 3)
			{
		...
		this.ShowMap(CustomSelectWindow.selectMapIndex, false, true);
	}
	...
	private void ShowMap(int index, bool imme = false, bool bSetGB = false)
	{
		...
		CustomSelectWindow.selectMapIndex = index;
		if (CustomSelectWindow.selectMapIndex == 0)
		{
		...
		float subNumAlpha = (float)(-(float)CustomSelectWindow.selectMapIndex) * this.numPositionInterval.x;
	...
	{
		this.bIsShowMap = false;
		float x = -this.changePageLength * (float)CustomSelectWindow.selectMapIndex;
		float x2 = -this.changePageLength * (float)(CustomSelectWindow.selectMapIndex + 1);
		Vector3 localPosition = this.numParent.transform.localPosition;
		Vector3 localPosition2 = this.numParent.transform.localPosition;
		localPosition2.x = x;
		localPosition.x = x2;
	...
	{
		if (CustomSelectWindow.selectMapIndex == 0 || CustomSelectWindow.selectMapIndex > this.MapIndexMax)
		{
			return;
		}
		...
		string matchId = this.mapList[CustomSelectWindow.selectMapIndex];
		Solarmax.Singleton<LocalPlayer>.Get().playerData.singleFightNext = (CustomSelectWindow.selectMapIndex == this.MapIndexMax);
	...
	private void OnNumClick(GameObject go)
	{
		...
		if (num == 0 && this.MapIndexMax != CustomSelectWindow.selectMapIndex)
		{
		...
		if (num == CustomSelectWindow.selectMapIndex)
		{
			return;
		}
		this.deltaScroll.x = 0f;
