...
namespace ProtoBuf.Meta
{
	public class MetaType : ISerializerProxy
	{
		internal MetaType(RuntimeTypeModel model, Type type)
		{
			...
			WireType wireType;
			IProtoSerializer protoSerializer = ValueMember.TryGetCoreSerializer(model, DataFormat.Default, type, out wireType, false, false, false, false);
		...
		private IProtoTypeSerializer BuildSerializer()
		{
			...
			else
			{
				...
				if (!this.IsAutoTuple)
				{
					...
					if (num != 0)
					{
						IEnumerator enumerator = this.subTypes.GetEnumerator();
						try
						{
					...
					if (count != 0)
					{
						IEnumerator enumerator2 = this.fields.GetEnumerator();
						try
						{
		...
		private static ProtoMemberAttribute NormalizeProtoMember(TypeModel model, MemberInfo member, MetaType.AttributeFamily family, bool forced, bool isEnum, BasicList partialMembers, int dataMemberOffset, bool inferByTagName)
		{
			...
			if (!flag && !flag2)
			{
				...
				if (!flag && attribute != null)
				{
					...
					MetaType.GetFieldBoolean(ref asReference, attribute, "AsReference");
					MetaType.GetFieldBoolean(ref dynamicType, attribute, "DynamicType");
					tagIsPinned = (flag2 = (num > 0));
				}
				if (!flag2 && partialMembers != null)
				{
					IEnumerator enumerator = partialMembers.GetEnumerator();
					try
					{
						while (enumerator.MoveNext())
						{
		...
		private int GetNextFieldNumber()
		{
			...
			if (this.subTypes != null)
			{
				IEnumerator enumerator2 = this.subTypes.GetEnumerator();
				try
				{
					while (enumerator2.MoveNext())
					{
