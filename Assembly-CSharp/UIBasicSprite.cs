...
public abstract class UIBasicSprite : UIWidget
{
	...
	public float fillAmount
	{
		...
		set
		{
			...
			{
				if (this.fillDirection == UIBasicSprite.FillDirection.RadialMuti)
				{
					this.mFillAmount = 0.5f;
				}
				else
				{
	...
	private void FilledFill(BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols)
	{
		...
		if (this.mFillAmount < 1f)
		{
			...
			if (this.mFillDirection == UIBasicSprite.FillDirection.RadialCircular)
			{
				for (int n = 0; n < 4; n++)
				{
					float t9;
					...
					UIBasicSprite.mTempPos[0].x = Mathf.Lerp(drawingDimensions.x, drawingDimensions.z, t9);
					UIBasicSprite.mTempPos[1].x = UIBasicSprite.mTempPos[0].x;
					UIBasicSprite.mTempPos[2].x = Mathf.Lerp(drawingDimensions.x, drawingDimensions.z, t10);
					UIBasicSprite.mTempPos[3].x = UIBasicSprite.mTempPos[2].x;
					UIBasicSprite.mTempPos[0].y = Mathf.Lerp(drawingDimensions.y, drawingDimensions.w, t11);
					UIBasicSprite.mTempPos[1].y = Mathf.Lerp(drawingDimensions.y, drawingDimensions.w, t12);
					UIBasicSprite.mTempPos[2].y = UIBasicSprite.mTempPos[1].y;
					UIBasicSprite.mTempPos[3].y = UIBasicSprite.mTempPos[0].y;
					UIBasicSprite.mTempUVs[0].x = Mathf.Lerp(drawingUVs.x, drawingUVs.z, t9);
					UIBasicSprite.mTempUVs[1].x = UIBasicSprite.mTempUVs[0].x;
					UIBasicSprite.mTempUVs[2].x = Mathf.Lerp(drawingUVs.x, drawingUVs.z, t10);
					UIBasicSprite.mTempUVs[3].x = UIBasicSprite.mTempUVs[2].x;
					UIBasicSprite.mTempUVs[0].y = Mathf.Lerp(drawingUVs.y, drawingUVs.w, t11);
					UIBasicSprite.mTempUVs[1].y = Mathf.Lerp(drawingUVs.y, drawingUVs.w, t12);
					UIBasicSprite.mTempUVs[2].y = UIBasicSprite.mTempUVs[1].y;
					UIBasicSprite.mTempUVs[3].y = UIBasicSprite.mTempUVs[0].y;
					float value3 = (n != 1 && n != 2) ? ((!this.mInvert) ? (this.mFillAmount * 2f) : (this.mFillAmount * 2f - 1f)) : ((!this.mInvert) ? (this.mFillAmount * 2f - 1f) : (this.mFillAmount * 2f));
					if (UIBasicSprite.RadialCut(UIBasicSprite.mTempPos, UIBasicSprite.mTempUVs, Mathf.Clamp01(value3), (!this.mInvert) ? (n < 2) : (n > 1), NGUIMath.RepeatIndex(n + 2, 4)))
					{
						for (int num3 = 0; num3 < 4; num3++)
						{
							verts.Add(UIBasicSprite.mTempPos[num3]);
							uvs.Add(UIBasicSprite.mTempUVs[num3]);
							cols.Add(drawingColor);
						}
					}
				}
			...
			if (this.mFillDirection == UIBasicSprite.FillDirection.RadialMuti)
			{
				if (this.spriteData == null || this.spriteData.Count <= 0)
				{
					return;
				}
				float num4 = 1f - this.spriteData[0].rate * 0.5f;
				for (int num5 = this.spriteData.Count - 1; num5 != 0; num5--)
				{
					this.DrawCircle(verts, uvs, cols, UIBasicSprite.mTempPos, UIBasicSprite.mTempUVs, num4, num5, this.spriteData[num5].color);
					num4 -= this.spriteData[num5].rate;
				}
				this.DrawCircle(verts, uvs, cols, UIBasicSprite.mTempPos, UIBasicSprite.mTempUVs, this.spriteData[0].rate, 0, this.spriteData[0].color);
				return;
			}
		}
		for (int num6 = 0; num6 < 4; num6++)
		{
			verts.Add(UIBasicSprite.mTempPos[num6]);
			uvs.Add(UIBasicSprite.mTempUVs[num6]);
			cols.Add(drawingColor);
		}
	}
	...
	private bool DrawCircle(BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols, Vector2[] tempPos, Vector2[] tempUV, float angle, int idx, Color color)
	{
		Vector4 drawingDimensions = this.drawingDimensions;
		Vector4 drawingUVs = this.drawingUVs;
		for (int i = 0; i < 4; i++)
		{
			...
			tempPos[0].x = Mathf.Lerp(drawingDimensions.x, drawingDimensions.z, t);
			tempPos[1].x = tempPos[0].x;
			tempPos[2].x = Mathf.Lerp(drawingDimensions.x, drawingDimensions.z, t2);
			tempPos[3].x = tempPos[2].x;
			tempPos[0].y = Mathf.Lerp(drawingDimensions.y, drawingDimensions.w, t3);
			tempPos[1].y = Mathf.Lerp(drawingDimensions.y, drawingDimensions.w, t4);
			tempPos[2].y = tempPos[1].y;
			tempPos[3].y = tempPos[0].y;
			tempUV[0].x = Mathf.Lerp(drawingUVs.x, drawingUVs.z, t);
			tempUV[1].x = tempUV[0].x;
			tempUV[2].x = Mathf.Lerp(drawingUVs.x, drawingUVs.z, t2);
			tempUV[3].x = tempUV[2].x;
			tempUV[0].y = Mathf.Lerp(drawingUVs.y, drawingUVs.w, t3);
			tempUV[1].y = Mathf.Lerp(drawingUVs.y, drawingUVs.w, t4);
			...
			{
				flag = !flag;
				value = ((!flag) ? (angle * 4f - (float)(3 - NGUIMath.RepeatIndex(i + 2, 4))) : (angle * 4f - (float)NGUIMath.RepeatIndex(i + 2, 4)));
			}
			else
			{
				value = ((i != 1 && i != 2) ? (angle * 2f - 1f) : (angle * 2f));
			}
			if (UIBasicSprite.RadialCut(tempPos, tempUV, Mathf.Clamp01(value), (idx != 0) ? flag : (i > 1), NGUIMath.RepeatIndex(i + 2, 4)))
			{
				for (int j = 0; j < 4; j++)
				{
					verts.Add(tempPos[j]);
	...
	public void AddSpriteData(int idx, SpriteData sp)
	{
		...
		{
			this.spriteData = new List<SpriteData>();
		}
		this.spriteData.Insert(idx, sp);
		this.fillAmount = 0.5f;
		this.mChanged = true;
	}
	...
	[HideInInspector]
	[SerializeField]
	protected List<SpriteData> spriteData;
