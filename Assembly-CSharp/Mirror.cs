...
public class Mirror : MonoBehaviour
{
	public void OnWillRenderObject()
	{
		...
		if (Mirror.s_InsideRendering)
		{
			return;
		}
		Mirror.s_InsideRendering = true;
		Camera camera;
		this.CreateMirrorObjects(current, out camera);
		Vector3 position = base.transform.position;
		Vector3 vector;
		if (this.m_IsFlatMirror)
		{
		...
		Mirror.CalculateReflectionMatrix(ref zero, plane);
		...
		Mirror.CalculateObliqueMatrix(ref projectionMatrix, clipPlane);
		...
		foreach (Material material in sharedMaterials)
		{
			...
			{
				material.SetTexture("_Ref", this.m_ReflectionTexture);
			}
		}
		if (this.m_DisablePixelLights)
		{
		...
		Mirror.s_InsideRendering = false;
	}
	...
	{
		Vector4 b = projection.inverse * new Vector4(Mirror.sgn(clipPlane.x), Mirror.sgn(clipPlane.y), 1f, 1f);
