...
public class UIScrollView : MonoBehaviour
{
	...
	public bool RestrictToStart(bool instant, bool horizontal, bool vertical)
	{
		...
		Vector2 vector = new Vector2(bounds.min.x, bounds.min.y);
		Vector2 vector2 = new Vector2(bounds.max.x, bounds.max.y);
		...
		if (this.mPanel.softBorderPadding && this.mPanel.clipping == UIDrawCall.Clipping.SoftClip)
		{
			vector3.x += this.mPanel.clipSoftness.x;
			vector3.y += this.mPanel.clipSoftness.y;
			vector4.x -= this.mPanel.clipSoftness.x;
			vector4.y -= this.mPanel.clipSoftness.y;
		}
		...
		if (horizontal && vector5.x > vector6.x)
		{
			return false;
		}
		if (vertical && vector5.y > vector6.y)
		{
			return false;
		}
		...
		if (horizontal && zero.x > 0f)
		{
			return false;
		}
		if (vertical && zero.y < 0f)
		{
			return false;
		}
		if (!horizontal)
		...
		if (zero.sqrMagnitude > 0.1f)
		{
			if (!instant && this.dragEffect == UIScrollView.DragEffect.MomentumAndSpring)
			{
				Vector3 pos = this.mTrans.localPosition + zero;
				pos.x = Mathf.Round(pos.x);
				pos.y = Mathf.Round(pos.y);
				SpringPanel.Begin(this.mPanel.gameObject, pos, 8f);
			}
			...
			{
				this.MoveRelative(zero);
				if (Mathf.Abs(zero.x) > 0.01f)
				{
					this.mMomentum.x = 0f;
				}
				if (Mathf.Abs(zero.y) > 0.01f)
				{
					this.mMomentum.y = 0f;
				}
				if (Mathf.Abs(zero.z) > 0.01f)
				{
					this.mMomentum.z = 0f;
				}
				this.mScroll = 0f;
	...
	public void Press(bool pressed)
	{
		...
		if (base.enabled && NGUITools.GetActive(base.gameObject))
		{
			...
			{
				bool flag = false;
				if (this.restrictToStart && this.mPanel.clipping != UIDrawCall.Clipping.None)
				{
					flag = this.RestrictToStart(this.dragEffect == UIScrollView.DragEffect.None, this.canMoveHorizontally, this.canMoveVertically);
				}
				bool flag2 = false;
				if (!flag && this.restrictWithinPanel && this.mPanel.clipping != UIDrawCall.Clipping.None)
				{
					flag2 = this.RestrictWithinBounds(this.dragEffect == UIScrollView.DragEffect.None, this.canMoveHorizontally, this.canMoveVertically);
				}
				...
				if (flag2 && this.onShowMore != null && this.CheckShowMore())
				{
					this.onShowMore();
				}
				if (flag2 && this.onShowLess != null && this.CheckShowLess())
				{
					this.onShowLess();
				}
			}
		...
		if (!this.mPressed)
		{
			if (this.mMomentum.magnitude > 0.0001f || this.mScroll != 0f)
			{
				...
				if (this.restrictToStart && this.mPanel.clipping != UIDrawCall.Clipping.None)
				{
					...
					{
						flag3 = this.RestrictToStart(false, this.canMoveHorizontally, this.canMoveVertically);
					}
				}
				if (!flag3 && this.restrictWithinPanel && this.mPanel.clipping != UIDrawCall.Clipping.None)
				{
					if (NGUITools.GetActive(this.centerOnChild))
					{
						if (this.centerOnChild.nextPageThreshold != 0f)
	...
	private bool CheckShowMore()
	{
		bool result = false;
		float deltaTime = RealTime.deltaTime;
		Vector3 a = this.mMomentum;
		if (a.magnitude > 0.0001f || this.mScroll != 0f)
		{
			if (this.movement == UIScrollView.Movement.Horizontal)
			{
				a -= this.mTrans.TransformDirection(new Vector3(this.mScroll * 0.05f, 0f, 0f));
			}
			else if (this.movement == UIScrollView.Movement.Vertical)
			{
				a -= this.mTrans.TransformDirection(new Vector3(0f, this.mScroll * 0.05f, 0f));
			}
			else if (this.movement == UIScrollView.Movement.Unrestricted)
			{
				a -= this.mTrans.TransformDirection(new Vector3(this.mScroll * 0.05f, this.mScroll * 0.05f, 0f));
			}
			else
			{
				a -= this.mTrans.TransformDirection(new Vector3(this.mScroll * this.customMovement.x * 0.05f, this.mScroll * this.customMovement.y * 0.05f, 0f));
			}
			Vector3 vector = NGUIMath.SpringDampen(ref a, this.dampenStrength, deltaTime);
			vector = this.mTrans.InverseTransformPoint(vector);
			vector -= this.mTrans.InverseTransformPoint(Vector3.zero);
			...
			else if (this.movement == UIScrollView.Movement.Horizontal && vector.x < 0f)
			{
				result = true;
			}
		}
	...
	public UIScrollView.OnDragNotification onShowMore;

	public UIScrollView.OnDragNotification onShowLess;
