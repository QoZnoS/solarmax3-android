...
public class SacrificeSkill : BaseNewSkill
{
	public override bool OnCast(Team castTeam)
	{
		...
		if (this.owerNode.GetShipFactCount((int)castTeam.team) < int.Parse(data.arg0))
		{
			return false;
		}
		bool flag = false;
			...
			if (node != this.owerNode)
			{
				if (node.currentTeam.Valid())
				{
					if (node.currentTeam != castTeam)
					{
						...
						{
							if (node.GetShipFactCount((int)node.team) != 0)
							{
		...
		if (flag)
		{
			this.owerNode.BombShipNum(this.owerNode.team, TEAM.Neutral, int.Parse(baseNewBuff.config.arg0));
		}
		base.PlaySkillAudio();
